...
public class PregnancyLuYinAddActivity extends Activity {
	...
	private boolean isRecoding = false; // 是否正在录音
			...
			if (msg.what == 1) {
				mediaPlayer.setOnCompletionListener(new OnCompletionListener() {
					...
					public void onCompletion(MediaPlayer mp) {
						play_stor_cb
								.setBackgroundResource(R.drawable.pregnancy_recoder_play);
				...
				currentTimeTv.setText("00:00:00");
				totalTimeTv
						.setText(TimeSwitcher.getInstance()
								.formatLongToTimeStrEn(
	...
	private void initViews() {
		...
		start_pause_cb
				.setBackgroundResource(R.drawable.pregnancy_recoder_start);
		start_pause_cb
				.setOnCheckedChangeListener(new OnCheckedChangeListener() {
					...
					public void onCheckedChanged(CompoundButton buttonView,
						...
						if (isChecked) {
							// 开始录音
							if (!isStop) {
								fileName = DateFormater.getInstatnce()
										.getCurrentDate();
							}
							start_pause_cb
									.setBackgroundResource(R.drawable.pregnancy_recoder_pause);
							...
							String recodeFilePath = recodeSavePath
									+ File.separator + recodeName + ".amr";
							soundMeter.start(recodeSavePath, recodeName
									+ ".amr");
		...
		play_stor_cb.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView,
				...
				if (isChecked) {
					...
					play_stor_cb
							.setBackgroundResource(R.drawable.pregnancy_recoder_stop);
					animationDrawable.start();
		...
		seekBar.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
			...
			public void onStopTrackingTouch(SeekBar seekBar) {
				...
				currentTimeTv.setText(TimeSwitcher.getInstance()
						.formatLongToTimeStrEn((long) seekBar.getProgress()));
			}
	...
	private class SeekBarTask extends AsyncTask<Integer, Integer, String> {
		...
		protected void onProgressUpdate(Integer... values) {
			...
			currentTimeTv.setText(TimeSwitcher.getInstance()
					.formatLongToTimeStrEn((long) values[0]));
			if (!isPlaying) {
				seekBar.setProgress(0);
				currentTimeTv.setText("00:00:00");
			}
	...
	private void updateTime() {
		...
		time.setText(TimeSwitcher.getInstance().formatLongToTimeStr(
				endMs - startMs + totalMs)); // 当前秒数减去开始录音时的秒数加上上次暂停前的秒数
	}
	...
	private void updateDisplay(double signalEMA) {
		...
		case 1:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_1);
			break;
		case 2:
		case 3:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_2);
			break;
		case 4:
		case 5:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_3);
			break;
		case 6:
		case 7:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_4);
			break;
		case 8:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_5);
		case 9:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_6);
			break;
		case 10:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_7);
		case 11:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_8);
			break;
		default:
			imageView
					.setImageResource(R.drawable.common_record_tone_highlight_9);
			break;
		}
	}
	...
	private void back() {
		...
		if (isRecoding) {
			Toast.makeText(this, "正在录音，请先暂停！", 0).show();
		} else {
	...
	public void btnClick(View view) {
		...
		case R.id.stop:
			...
			final String filePath = recodeSavePath + File.separator + fileName
					+ ".amr";
			...
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					MutilRecodeManager.getInstance().mergeAllAmrFiles(
							recodeFiles, filePath);
	...
	private class SaveLuYinThread extends Thread {
		...
		public void run() {
			...
			CopyFileFromData.getInstance().copyDatabase(
					PregnancyLuYinAddActivity.this, "pregnancy_diary.db");
	...
	private void mediaStop(boolean isSave) {
		...
		if (!isSave) {
			File file = new File(recodeSavePath + File.separator + fileName
					+ ".amr");
			if (file.exists()) {
				file.delete();
			}
